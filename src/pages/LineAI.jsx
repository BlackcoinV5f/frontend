...
const API_URL = `${BASE_URL}/api/lineai`; // ✅ endpoint LineAI
...
const LineAI = () => {
  ...
  useEffect(() => {
    ...
    const cache = { messages, savedAt: Date.now() };
    localStorage.setItem("lineai_messages", JSON.stringify(cache)); // ✅ clé séparée
  }, [messages]);
  ...
  useEffect(() => {
    ...
    const name = user?.username || user?.name || user?.firstName || "Utilisateur";

    const cached = localStorage.getItem("lineai_messages"); // ✅ clé séparée
    ...
    if (cached) {
      try {
        ...
        if (Date.now() - savedAt < heures24) {
          ...
          localStorage.removeItem("lineai_messages");
        }
      } catch {
        localStorage.removeItem("lineai_messages");
      }
    ...
    setMessages([
      {
        ...
        text:
          `${getGreeting()} **${name}** 👋\n\n` +
          `Je suis **Line AI**, votre assistant intelligent.\n\n` + // ✅ Line AI
  ...
  const handleClearCache = () => {
    localStorage.removeItem("lineai_messages"); // ✅ clé séparée
  ...
  return (
    <div className="lineai-container"> {/* ✅ classe séparée */}
      ...
      <div className="chat-header">
        <div className="header-left">
          ...
          <div className="header-avatar">💡</div> {/* ✅ icône différente */}

          <div className="header-info">
            <h2>Line AI</h2> {/* ✅ nom différent */}
